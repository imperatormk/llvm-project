...
byte-identical to it for the kernels in the LLVM AIR test suite. Field
names mirror the implementation in
``llvm/lib/Target/AIRWriter/AIRLibWriter.{h,cpp}``.
...
The 88-byte header is::
    ...
    ------   ----   --------------   --------------------------------
      ...
      14       1   OSMajor          (e.g. ``26`` — see DefaultAIRLibOSMajor)
      15       3   OSPad            zero
...
* ``AIREmbedderPass`` builds the metallib bytes from a Module.
* ``AIRLibObjectWriter`` (``llvm/lib/MC/AIRLibObjectWriter.cpp``) wraps
  those bytes into ``-filetype=obj`` output.
* ``MCAIRLibStreamer`` (``llvm/lib/MC/MCAIRLibStreamer.cpp``) plumbs
