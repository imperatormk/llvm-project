; AIRPrepare inserts a base GEP (__base_<name>) at the entry of any function
...
;
; TODO(air-revisit GH-XXX): the eventual goal is to retype the global into
; the inferred element type (e.g. [32 x float]) and emit float-stride GEPs;
; the rewrite here is a documented intermediate state.
;
; RUN: llc -mtriple=air -stop-after=air-prepare %s -o - | FileCheck %s
