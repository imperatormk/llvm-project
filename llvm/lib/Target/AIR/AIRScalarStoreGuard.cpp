//===- AIRScalarStoreGuard.cpp - Guard scalar device stores -------------===//
//
...
#define DEBUG_TYPE "air-scalar-store-guard"

// AIR device address space. Inlined here since this pass only needs the
// one constant.
...
PreservedAnalyses AIRScalarStoreGuardPass::run(Module &M,
                                                 ModuleAnalysisManager &AM) {
  return scalarStoreGuard(M) ? PreservedAnalyses::none()
                             : PreservedAnalyses::all();
}

bool AIRScalarStoreGuardLegacy::runOnModule(Module &M) {
  return scalarStoreGuard(M);
}

char AIRScalarStoreGuardLegacy::ID = 0;

INITIALIZE_PASS(AIRScalarStoreGuardLegacy, DEBUG_TYPE,
                "AIR Scalar Store Guard", false, false)

ModulePass *llvm::createAIRScalarStoreGuardLegacyPass() {
  return new AIRScalarStoreGuardLegacy();
}
