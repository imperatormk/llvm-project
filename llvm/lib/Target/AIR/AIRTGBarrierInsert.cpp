//===- AIRTGBarrierInsert.cpp - Insert TG memory barriers ---------------===//
//
...
#define DEBUG_TYPE "air-tg-barrier-insert"
  ...
  for (Function &F : M) {
    ...
    for (BasicBlock &BB : F) {
      ...
      for (unsigned I = 0; I < Term->getNumSuccessors(); ++I) {
        ...
        if (isa<UncondBrInst, CondBrInst>(SuccTerm)) {
      ...
      if (SuccHasTGStore) {
        ...
        // crashes AIR's GPU JIT. Split off so they live in separate blocks.
...
PreservedAnalyses AIRTGBarrierInsertPass::run(Module &M,
                                                ModuleAnalysisManager &AM) {
  return tgBarrierInsert(M) ? PreservedAnalyses::none()
                            : PreservedAnalyses::all();
}

bool AIRTGBarrierInsertLegacy::runOnModule(Module &M) {
  return tgBarrierInsert(M);
}

char AIRTGBarrierInsertLegacy::ID = 0;

INITIALIZE_PASS(AIRTGBarrierInsertLegacy, DEBUG_TYPE,
                "AIR Threadgroup Barrier Insertion", false, false)

ModulePass *llvm::createAIRTGBarrierInsertLegacyPass() {
  return new AIRTGBarrierInsertLegacy();
}
