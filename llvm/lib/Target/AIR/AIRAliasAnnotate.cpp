//===- AIRAliasAnnotate.cpp - AIR-style aliasing metadata ---------------===//
//
...
#define DEBUG_TYPE "air-alias-annotate"
...
PreservedAnalyses AIRAliasAnnotatePass::run(Module &M,
                                              ModuleAnalysisManager &AM) {
  return runImpl(M) ? PreservedAnalyses::none() : PreservedAnalyses::all();
}

bool AIRAliasAnnotateLegacy::runOnModule(Module &M) { return runImpl(M); }

char AIRAliasAnnotateLegacy::ID = 0;

INITIALIZE_PASS(AIRAliasAnnotateLegacy, DEBUG_TYPE,
                "AIR-style alias-metadata annotate", false, false)

ModulePass *llvm::createAIRAliasAnnotateLegacyPass() {
  return new AIRAliasAnnotateLegacy();
}
