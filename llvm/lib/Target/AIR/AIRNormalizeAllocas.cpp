//===- AIRNormalizeAllocas.cpp - Pre-serialization IR cleanup ----------===//
//
...
#define DEBUG_TYPE "air-normalize-allocas"
...
static bool normalizeAllocas(Module &M) {
  ...
  // Strip 'disjoint' flag from 'or' instructions (AIR v1 bitcode).
  for (Function &F : M)
    for (BasicBlock &BB : F)
...
PreservedAnalyses AIRNormalizeAllocasPass::run(Module &M,
                                                 ModuleAnalysisManager &AM) {
  return normalizeAllocas(M) ? PreservedAnalyses::none()
                             : PreservedAnalyses::all();
}

bool AIRNormalizeAllocasLegacy::runOnModule(Module &M) {
  return normalizeAllocas(M);
}

char AIRNormalizeAllocasLegacy::ID = 0;

INITIALIZE_PASS(AIRNormalizeAllocasLegacy, DEBUG_TYPE,
                "AIR Normalize Allocas", false, false)

ModulePass *llvm::createAIRNormalizeAllocasLegacyPass() {
  return new AIRNormalizeAllocasLegacy();
}
