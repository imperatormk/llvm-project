...
static bool retypeByteGlobals(Module &M) {
  ...
  for (auto *GV : ByteGlobals) {
    ...
    // scalar type — Metal's typed bitcode rejects the mismatched pointer at
    // validate time.
...
// The Metal GPU JIT rejects the type mismatch when an i8-source GEP is derived
...
// The Metal GPU JIT crashes on non-float typed TG pointers when float-typed
...
static bool rewriteTGGlobalGEPs(Module &M) {
  ...
  // Metal 4 (macOS 26): the toolchain emits `store <N x T>` on threadgroup
  // memory intact.
